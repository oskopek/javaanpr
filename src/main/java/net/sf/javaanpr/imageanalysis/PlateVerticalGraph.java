...
public class PlateVerticalGraph extends Graph {
	...
	Intelligence.configurator
			.getDoubleProperty("plateverticalgraph_peakfootconstant");
	...
	public class PeakComparer implements Comparator<Object> {
		...
		private float getPeakValue(Object peak) {
			...
			// vyska peaku
			return graphHandle.yValues.elementAt(((Peak) peak).getCenter());
	...
	public Vector<Peak> findPeak(int count) {
		...
		for (int i = 0; i < yValues.size(); i++) {
			yValues.set(i, yValues.elementAt(i) - this.getMinValue());
		}
		...
		for (int c = 0; c < count; c++) { // for count
			...
			int leftIndex = indexOfLeftPeakRel(maxIndex,
					PlateVerticalGraph.peakFootConstant);
